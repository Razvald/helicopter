...
class XFeat(nn.Module):
	...
	def __init__(self, weights = os.path.abspath(os.path.dirname(__file__)) + '/../weights/xfeat.pt', top_k = 4096, detection_threshold=0.05, dev='cpu'):
		super().__init__()
		self.dev = dev
		self.session = ort.InferenceSession("net.onnx", providers=['CPUExecutionProvider'])
		self.top_k = top_k
	...
	def detectAndCompute(self, x, top_k = None, detection_threshold = None):
		...
		M1, K1, H1 = self.session.run(None, {'input': x.numpy()})
		M1, K1, H1 = torch.tensor(M1), torch.tensor(K1), torch.tensor(H1) 
