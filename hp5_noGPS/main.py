...
from gps_utils import gps2pixhawk
...
def initialize_processes(stop, vcap,  posque, vidque, posque_window):
    ...
    # info_on_display_procecc = Process(target=send_info_on_display, args=(stop, posque_window))
    # info_on_display_procecc.start()
    
    return data_poll_process, video_poll_process
    ...
    with NamedWindow("Info"):
        ...
        while not stop_event.is_set():
            try:
                ...
                if monitor_exists:
                    img = cv2.resize(frame, (screen_width, screen_height), interpolation = cv2.INTER_AREA)
                    img = info_on_image.show_info_on_display(img, msg)
                    cv2.imshow('Info', img)
                    cv2.waitKey(1)
...
def main_loop(master, stop, posque, vidque, vis_odo, pos2pixhawk_func, data_dir, pos_queue_window):
    print("main_loop")
    voltage = None
    init = True
    start_imu = 0
...
if __name__ == '__main__':
    ...
    processes = initialize_processes(stop_event, vcap, pos_queue, video_queue, pos_queue_window)

    # Get initial GPS coordinates
    latitude, longitude, altitude = get_initial_coordinates()
